...
import type { RenderLabel } from '../date-picker-view/date-picker-view.vue'
import type { PickerColumn } from '../picker-view/index'
...
export function generateDatePickerColumns(
  ...
  const generateColumn = (from: number, to: number, precision: DatePrecision) => {
    let column: number[] = []
...
export function convertDateToStringArray(date: Date | undefined | null): string[] {
  if (!date) return []
  return [
...
export function convertStringArrayToDate<T extends string | number | null | undefined>(
  value: T[],
): Date {
